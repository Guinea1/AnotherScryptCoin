...
const char * const BITCOIN_CONF_FILENAME = "anotherscryptcoin.conf";
...
#else
    const char* pszModule = "anotherscryptcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AnotherScryptCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AnotherScryptCoin";
#else
    // Unix
    return pathRet / ".anotherscryptcoin";
#endif
#endif
}
