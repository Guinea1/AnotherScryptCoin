...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("anotherscryptcoin"))
        return false;
...
{
    QString ret = QString("anotherscryptcoin:%1").arg(info.address);
    int paramCount = 0;
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "anotherscryptcoin.desktop";
    return GetAutostartDir() / strprintf("anotherscryptcoin-%s.lnk", chain);
}
