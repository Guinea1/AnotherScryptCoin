# PSBT Howto for AnotherScryptCoin Core

Since AnotherScryptCoin Core 0.17, an RPC interface exists for Partially Signed AnotherScryptCoin
...
PSBT is an interchange format for AnotherScryptCoin transactions that are not fully signed
...
## PSBT in AnotherScryptCoin Core
...
#### Multisig with multiple AnotherScryptCoin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
AnotherScryptCoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `anotherscryptcoin-cli` is used.
