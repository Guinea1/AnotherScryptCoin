AnotherScryptCoin Core version *0.15.1* is now available from:

  <https://download.anotherscryptcoin.org/anotherscryptcoin-0.15.1/>
...
A number of changes to the way AnotherScryptCoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 bytes per kB (0.00001 ASC). This is to prepare for dropping the minimum transaction fee to 0.1 bytes per kB (0.0001 ASC) in the next release.
